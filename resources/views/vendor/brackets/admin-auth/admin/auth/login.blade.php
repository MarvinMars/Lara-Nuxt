						...
						<auth-form
							...
							<form class="form-horizontal" role="form" method="POST" action="{{ url('/admin/login') }}" novalidate>
								...
								<div class="auth-body">
									...
									<div class="form-group" :class="{'has-danger': veeErrors.has('email'), 'has-success': this.veeFields.email && this.veeFields.email.valid }">
										...
										<div class="input-group input-group--custom">
											...
											<input type="text" v-model="form.email" v-validate="'required|email'" class="form-control" :class="{'form-control-danger': veeErrors.has('email'), 'form-control-success': this.veeFields.email && this.veeFields.email.valid}" id="email" name="email" placeholder="{{ trans('brackets/admin-auth::admin.auth_global.email') }}">
										</div>
										<div v-if="veeErrors.has('email')" class="form-control-feedback form-text" v-cloak>@{{ veeErrors.first('email') }}</div>
									</div>

									<div class="form-group" :class="{'has-danger': veeErrors.has('password'), 'has-success': this.veeFields.password && this.veeFields.password.valid }">
										...
										<div class="input-group input-group--custom">
											...
											<input type="password" v-model="form.password"  class="form-control" :class="{'form-control-danger': veeErrors.has('password'), 'form-control-success': this.veeFields.password && this.veeFields.password.valid}" id="password" name="password" placeholder="{{ trans('brackets/admin-auth::admin.auth_global.password') }}">
										</div>
										<div v-if="veeErrors.has('password')" class="form-control-feedback form-text" v-cloak>@{{ veeErrors.first('password') }}</div>
									</div>
